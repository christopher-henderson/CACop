...
var letsencryptChain []*x509.Certificate
var entrustRevokedChain []*x509.Certificate

func init() {
	letsencryptChain = buildCertChain([][]byte{letsencrypt, letsencryptIntermediate, letsencryptRoot})
	entrustRevokedChain = buildCertChain([][]byte{entrustRevokedLeaf, entrustRevokedIntermediate, entrustRevokedRoot})
}

func buildCertChain(chain [][]byte) (certChain []*x509.Certificate) {
	for _, raw := range chain {
		block, rest := pem.Decode(raw)
		if len(rest) != 0 {
			log.Panic(string(rest))
		}
...
func TestValidLeaf(t *testing.T) {
	leaf := letsencryptChain[0]
	intermediate := letsencryptChain[1]
	want := Good
	for i, got := range Query(leaf, intermediate) {
		if got != want {
			t.Errorf(`Got "%s" OCSP status,  want "%s"". OCSP responder is %s`, got, want, leaf.OCSPServer[i])
		}
	}
}

var entrustRevokedLeaf = []byte(`-----BEGIN CERTIFICATE-----
...
var entrustRevokedIntermediate = []byte(`-----BEGIN CERTIFICATE-----
...
var entrustRevokedRoot = []byte(`-----BEGIN CERTIFICATE-----
...
func TestRevokedLeaf(t *testing.T) {
	leaf := entrustRevokedChain[0]
	intermediate := entrustRevokedChain[1]
	want := Revoked
	t.Log(leaf.Subject)
	for i, got := range Query(leaf, intermediate) {
		if got != want {
